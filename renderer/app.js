...
const teamMembers = [
  'Anh Danh', 'Hiếu', 'Phúc', 'Quí',
  'Chi Trâm', 'Chị Diệu', 'Anh Tấn', 'Thái', 'Minh'
];
...
function updateUsersList() {
  ...
  if (count === 0) {
    usersListEl.innerHTML = '<div class="empty-state">No team members online</div>';
    return;
  }

  usersListEl.innerHTML = onlineUsers.map(user => {
...
function updateAlertsList() {
  ...
  alertsListEl.innerHTML = recentAlerts.slice(0, 10).map(alert => {
...
function sendAlert(bossDirection = 'boss1') {
  ...
  if (now - lastAlertTime < ALERT_COOLDOWN) {
    const remaining = Math.ceil((ALERT_COOLDOWN - (now - lastAlertTime)) / 1000);
    showToast(`Please wait ${remaining}s before sending another alert`, 'error');
    return;
  }
  ...
  showToast(`Alert sent: Boss ${bossDirection === 'boss1' ? '1' : '2'}!`, 'success');
}
